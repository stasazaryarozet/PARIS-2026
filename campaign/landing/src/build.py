...
def apply_russian_typography(text):
    ...
    # 5.3. После однобуквенных предлогов и союзов (включая а/А и и/И)
    text = re.sub(r'\b([авикосуяАВИКОСУЯ])\s+', r'\1&nbsp;', text)
...
def parse_content(md_path):
    ...
    # INCLUSIONS
    data['inclusions'] = []
    # Ограничиваем блок "Что включено" до следующего заголовка уровня ## или конца файла
    incl_section = re.search(r'## Что включено\n\n(.+?)(?=\n## |\Z)', body, re.DOTALL)
    if incl_section:
        # Каждая карточка: **<иконка> <заголовок>** [описание]
        # Описание завершается перед следующей карточкой, пустой строкой или новым секционным заголовком
        incl_pattern = r'\*\*([+−±€✓✗💶]) (.+?)\*\*(?:\s*\n(.+?))?(?=\n\*\*|\n## |\n\n|\Z)'
